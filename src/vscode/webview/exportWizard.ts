  ...
  <head>
    ...
    <style>
      ...
      *{box-sizing:border-box;scrollbar-width:thin;scrollbar-color:var(--scroll) transparent}
      ...
      .settings-grid{display:grid;grid-template-columns:1.1fr .9fr;gap:16px}.search-row{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:10px}.filter-grid,.date-grid,.split{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.split{grid-template-columns:repeat(2,minmax(0,1fr))}.toolbar,.footer-meta,.session-top,.relationship-box,.toggle-row,.child-header{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap}
      ...
      .input,.select,.datetime,.button,.primary-button,.toggle-button{border-radius:16px;border:1px solid var(--line);font:inherit}.input,.select,.datetime{width:100%;padding:12px 14px;background:rgba(255,255,255,.8);color:var(--text);outline:none}.button,.primary-button,.toggle-button{padding:12px 16px;cursor:pointer}.button,.toggle-button{background:rgba(255,255,255,.74);color:#547099}.primary-button{background:linear-gradient(135deg,#1f2937,#111827);color:#fff;border-color:transparent}.input:focus,.select:focus,.datetime:focus{border-color:rgba(91,146,247,.52);box-shadow:0 0 0 4px rgba(91,146,247,.12)}
      ...
      @media (max-width:1180px){.stats,.settings-grid,.filter-grid,.date-grid,.split{grid-template-columns:1fr}.search-row{grid-template-columns:1fr}.toolbar,.footer-meta,.session-top,.relationship-box,.toggle-row,.child-header{flex-direction:column;align-items:stretch}}
    </style>
  </head>
  <body>
    <div class="page">
      ...
      <section class="settings">
        ...
        <div class="action-card"><div class="status-line" id="status-line"></div><div class="footer-meta"><span id="selection-summary">已选 ${selectedCount} 条会话</span><span>当前渲染 ${renderedSessions.length} / ${input.sessions.length}</span></div><button class="primary-button" id="export-button">生成导出文件</button></div>
      </section>
    </div>
